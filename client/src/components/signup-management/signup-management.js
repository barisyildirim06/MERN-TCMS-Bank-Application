...
function SignupManagement({ users, onUserVerify }) {
    ...
    const createdAtFilterDropdown = <>
      <Select defaultValue={'week'} style={{ width: '200px', height: '5vh' }} onChange={(value) => handleChange(value,'createdAt')}>
    ...
    const filterVerifyDropdown = <>
      <Select defaultValue={'all'} style={{ width: '200px', height: '5vh' }} onChange={(value) => handleChange(value,'verify')}>
    ...
    //
    const data = users?.users?.filter(u => new Date(u.createdAt).valueOf() > (filterType.createdAt === 'day' ? oneDayUnix : sevenDaysUnix))
    .filter(u => {
        const { verify } = filterType 
        if (verify === 'all') return true;
        if (verify === 'verified' && u.verified) return true;
